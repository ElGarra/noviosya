...
export function GuestCRM({ guests: initial }: { guests: Guest[] }) {
  ...
  const [newGuest, setNewGuest]   = useState({ firstName: '', lastName: '', email: '', maxCompanions: 0, notes: '' })

  // ── Stats ────────────────────────────────────────────────────────────────
  const confirmed  = guests.filter((g) => g.status === 'CONFIRMED')
  const companions = confirmed.reduce((acc, g) => acc + g.companions.length, 0)
  const totalPeople = confirmed.length + companions
  ...
  const dietRows = useMemo(() => {
    ...
    for (const g of guests.filter((g) => g.status === 'CONFIRMED')) {
  ...
  async function addGuest(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setGuests([...guests, {
        ...data.guest,
        rsvpUrl: `${appUrl}/i/${data.guest.token}`,
        status: 'PENDING',
        companions: [],
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Pendientes',    value: guests.filter((g) => g.status === 'PENDING').length,  color: 'text-amber-600' },
          { label: 'Total personas', value: totalPeople, color: 'text-gold' },
        ].map(({ label, value, color }) => (
          <div key={label} className="bg-white p-4 text-center shadow-sm">
            <p className={`text-3xl font-serif font-light ${color ?? 'text-text-base'}`}>{value}</p>
            <p className="text-[0.6rem] tracking-[0.2em] uppercase text-text-muted mt-0.5">{label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
            ...
            return (
              <button key={key} onClick={() => setTab(key)}
                className={`px-3 py-1.5 text-xs tracking-wide rounded-full transition-colors ${
                  tab === key ? 'bg-gold text-white' : 'text-text-muted hover:text-gold'
                }`}>
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <button onClick={() => setShowDiets(!showDiets)}
            className={`text-xs px-3 py-1.5 border transition-colors ${showDiets ? 'border-gold bg-gold text-white' : 'border-gold/30 text-gold hover:border-gold'}`}>
      ...
      {showDiets && (
        <div className="bg-white border border-gold/20 p-5">
          <p className="text-[0.65rem] tracking-[0.2em] uppercase text-gold mb-3">Restricciones alimentarias — confirmados</p>
          {dietRows.length === 0 ? (
        ...
        <table className="w-full text-sm min-w-[700px]">
          <thead>
            <tr className="border-b border-gold/20 text-left">
              {['Nombre', 'Estado', 'Acompañantes', 'Dieta', 'Mesa', 'Acciones'].map((h) => (
          ...
          <tbody>
            {filtered.map((guest) => {
              const { label, className } = STATUS[guest.status]
              const allDiets = [
                ...(guest.dietaryRestrictions ? [guest.dietaryRestrictions] : []),
                ...guest.companions.filter((c) => c.dietaryRestrictions).map((c) => c.dietaryRestrictions!),
              ]
              ...
              return (
                <tr key={guest.id} className="border-b border-gold/10 hover:bg-cream/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    ...
                    {guest.message && (
                      <p className="text-[0.68rem] text-text-muted mt-0.5 italic">💬 "{guest.message}"</p>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    {allDiets.length > 0 ? (
                      <ul className="space-y-0.5">
                        {allDiets.map((d, i) => (
                          <li key={i} className="text-xs text-amber-700 bg-amber-50 px-2 py-0.5 rounded">
                            {d}
                          </li>
                        ))}
                      </ul>
                    ...
                    {editingTable === guest.id ? (
                      <div className="flex gap-1">
                        <input value={tableVal} onChange={(e) => setTableVal(e.target.value)}
                          ...
                          onKeyDown={(e) => { if (e.key === 'Enter') saveTable(guest.id); if (e.key === 'Escape') setEditingTable(null) }}
                          autoFocus />
                        <button onClick={() => saveTable(guest.id)}
                          className="text-gold text-xs hover:text-gold/70">✓</button>
                      </div>
                    ) : (
                      <button onClick={() => { setEditingTable(guest.id); setTableVal(guest.table ?? '') }}
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-10 text-center text-text-muted italic text-sm">
                  {search ? `Sin resultados para "${search}"` : 'No hay invitados en esta categoría.'}
                </td>
              </tr>
            )}
          </tbody>
